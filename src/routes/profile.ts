...
const ensureProfile = (userId: number) =>
  pool.query(
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    else callback(new Error('Only PNG/JPEG/WEBP images allowed'), false);
  },
});
...
// PUT /api/profile/me
const updateMe = async (req: AuthRequest, res: any) => {
  const userId = req.user!.sub;
...
router.post('/avatar', authMiddleware, upload.single('avatar'), async (req: AuthRequest & { file?: Express.Multer.File }, res) => {
  const userId = req.user!.sub;
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' });

  const newPublicPath = `/uploads/avatars/${req.file.filename}`;
  try {
    const prev = await pool.query(`SELECT avatar_url FROM profiles WHERE user_id = $1`, [userId]);
    const { rows } = await pool.query(
      `UPDATE profiles SET avatar_url = $2, updated_at = NOW()
         WHERE user_id = $1
         RETURNING user_id, avatar_url`,
      [userId, newPublicPath]
    );

    const oldPath = prev.rows?.[0]?.avatar_url ? path.join(process.cwd(), prev.rows[0].avatar_url) : null;
    if (oldPath && oldPath.startsWith(path.join(process.cwd(), 'uploads'))) {
...
router.delete('/avatar', authMiddleware, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    await pool.query(`UPDATE profiles SET avatar_url = NULL, updated_at = NOW() WHERE user_id = $1`, [userId]);

    if (old) {
      const fp = path.join(process.cwd(), old);
      if (fp.startsWith(path.join(process.cwd(), 'uploads'))) {
